...
test("non-member can become member with correct member password", async () => {
  const agent = request.agent(app);
...
test("member can become admin with correct admin password", async () => {
  const agent = request.agent(app);

  await pool.query("UPDATE users SET member = true WHERE email = $1", [
    "testor@test.com",
  ]);
...
test("member can reset their status with the correct reset password", async () => {
  const agent = request.agent(app);
